...
class WeeklyRankingJobE2ETest @Autowired constructor(
    ...
    @DisplayName("product_metrics_daily 일별 변화량을 가중치 기반으로 집계하여 주간 랭킹 MV에 적재한다.")
    @Test
    fun aggregatesDailyMetricsIntoWeeklyRanking() {
        // arrange
        val today = LocalDate.now()
        // product 1: score = 100*0.2 + 500*0.1 + 50*0.7 = 20 + 50 + 35 = 105
        insertDailyMetrics(1L, today, 100, 500, 50, 1_000_000)
        // product 2: score = 200*0.2 + 300*0.1 + 30*0.7 = 40 + 30 + 21 = 91
        insertDailyMetrics(2L, today, 200, 300, 30, 500_000)
        // product 3: score = 50*0.2 + 1000*0.1 + 80*0.7 = 10 + 100 + 56 = 166
        ...
        val rankings = mvWeeklyJpaRepository.findAll().sortedBy { it.rankPosition }
        assertAll(
    ...
    fun aggregatesMultipleDaysOfDeltasIntoScore() {
        // arrange
        val today = LocalDate.now()
        // product 1: day1(like=60, view=300, order=30) + day2(like=40, view=200, order=20) = total(100, 500, 50)
        // score = 100*0.2 + 500*0.1 + 50*0.7 = 20 + 50 + 35 = 105
        insertDailyMetrics(1L, today, 60, 300, 30, 600_000)
        insertDailyMetrics(1L, today.minusDays(1), 40, 200, 20, 400_000)

        // act
        val jobExecution = jobLauncherTestUtils.launchJob(buildJobParameters())

        // assert
        val rankings = mvWeeklyJpaRepository.findAll()
        assertAll(
            { assertThat(jobExecution.exitStatus.exitCode).isEqualTo(ExitStatus.COMPLETED.exitCode) },
    ...
    @DisplayName("7일 범위 밖의 데이터는 집계에서 제외된다.")
    @Test
    fun excludesDataOutsideWeeklyRange() {
        // arrange
        val today = LocalDate.now()
        insertDailyMetrics(1L, today, 100, 500, 50, 1_000_000)
        insertDailyMetrics(2L, today.minusDays(7), 200, 300, 30, 500_000) // 범위 밖
    ...
    @DisplayName("product_metrics_daily에 데이터가 없으면, MV 테이블에 적재하지 않는다.")
    @Test
    fun producesEmptyResult_whenNoMetrics() {
        // act
        val jobExecution = jobLauncherTestUtils.launchJob(buildJobParameters())
        ...
        assertAll(
            { assertThat(jobExecution.exitStatus.exitCode).isEqualTo(ExitStatus.COMPLETED.exitCode) },
            { assertThat(mvWeeklyJpaRepository.findAll()).isEmpty() },
        )
    }
}
